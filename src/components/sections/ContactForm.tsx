...
export function ContactForm() {
  ...
  return (
    <section id="kontakt" className="scroll-mt-16 border-t border-line py-12 md:py-24">
      <Container>
        <div className="grid gap-8 rounded-card border border-line bg-raised p-6 md:grid-cols-[.9fr_1.1fr] md:gap-12 md:p-12">
          <div>
            ...
            <h2 className="font-display text-display-lg-sm md:text-display-lg">Imate ideju? Pošaljite nam poruku</h2>
            <p className="mt-3 text-muted">Opišite ukratko šta vam treba. Javljamo se s konkretnim prijedlogom, bez obaveze.</p>
          </div>

          {status === "success" ? (
            <div role="status" className="grid content-center justify-items-start gap-3">
            ...
            <form onSubmit={onSubmit} className="grid gap-6" noValidate={false}>
              ...
              {status === "error" && (
                <p role="alert" className="text-sm font-medium text-signal">Greška: {message}</p>
              )}
