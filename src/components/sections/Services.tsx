...
export function Services() {
  return (
    <Section
      id="usluge"
      eyebrow="Usluge"
      title="Šta gradimo za vas"
      intro="Jedan tim za sve digitalno što vašem biznisu treba."
    >
      <div className="grid gap-6 md:grid-cols-3">
        {services.map(({ icon, title, text }) => (
          <article
            key={title}
            className="flex flex-col rounded-card border border-line bg-raised p-6 transition duration-150 hover:-translate-y-0.5 hover:border-line-strong"
          >
            <IconTile icon={icon} />
